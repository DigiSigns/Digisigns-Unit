...
func main() {
	...
	rows, err := db.Query("SELECT video_url FROM orders WHERE is_approved=TRUE AND start_date='"+getCurrTime()+"' AND device_id_pk=(SELECT id FROM devices WHERE devices.host_id="+SID+" AND devices.device_id="+UID+");")
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
